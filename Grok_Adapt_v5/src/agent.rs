//! EchoAgent - The core orchestrator for the Adapt Rust Agent Framework.
//!
...
impl EchoAgent {
    ...
    pub async fn run(&mut self) -> Result<()> {
        println!("Adapt: Ready. Type 'quit' or 'exit' to end session.\n");
    ...
    async fn process_turn(&mut self, user_input: &str) -> Result<String> {
	    ...
	    if self.messages.is_empty() ||
               self.messages.last().map_or(true, |m| m["role"] != "user") {
	...
	// === DEBUG ===
	let raw_resp = request.send().await?;

	let full_json: Value = raw_resp.json().await?;

	let response_text = full_json["output"]
                .as_array()
                .and_then(|arr| {
                    arr.iter().find(|item| {
                ...
                .or_else(|| full_json["output_text"].as_str())
